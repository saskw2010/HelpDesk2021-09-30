...
{
    public partial class EditIHelpDeskTicketwithdetailsComponent : ComponentBase
    {
        ...
        protected RadzenDataList<Testauth.Models.Authenticationconn.HelpDeskTicketDetail> datalist0;
        ...
        dynamic _HelpDeskTicketIdpar;
        protected dynamic HelpDeskTicketIdpar
        {
            get
            {
                return _HelpDeskTicketIdpar;
            }
            set
            {
                if (!object.Equals(_HelpDeskTicketIdpar, value))
                {
                    var args = new PropertyChangedEventArgs(){ Name = "HelpDeskTicketIdpar", NewValue = value, OldValue = _HelpDeskTicketIdpar };
                    _HelpDeskTicketIdpar = value;
                    OnPropertyChanged(args);
                    Reload();
                }
            }
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var authenticationconnGetHelpDeskTicketByIdResult = await Authenticationconn.GetHelpDeskTicketById(id:Convert.ChangeType(Id, Type.GetTypeCode(typeof(Int64))));
            helpdeskticket = authenticationconnGetHelpDeskTicketByIdResult;

            canEdit = authenticationconnGetHelpDeskTicketByIdResult != null;

            HelpDeskTicketIdpar = Id;
            ...
            if (Id != null)
            {
                var authenticationconnGetHelpDeskTicketDetailsResult = await Authenticationconn.GetHelpDeskTicketDetails(filter:$@"HelpDeskTicketId eq {Id}");
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Testauth.Models.Authenticationconn.HelpDeskTicket args)
        {
            try
            {
                var authenticationconnUpdateHelpDeskTicketResult = await Authenticationconn.UpdateHelpDeskTicket(id:Convert.ChangeType(Id, Type.GetTypeCode(typeof(Int64))), helpDeskTicket:helpdeskticket);
        ...
        {
            string HelpDeskTicketId = Convert.ChangeType(Id, Type.GetTypeCode(typeof(string)));
            var dialogResult = await DialogService.OpenAsync<AddHelpDeskTicketDetailmos>("Add Help Desk Ticket Detailmos", new Dictionary<string, object>() { { "HelpDeskTicketIdpar", helpdeskticket.Id } });
           
           //UriHelper.NavigateTo($"edit-i-help-desk-ticketwithdetails/{helpdeskticket.Id}",forceLoad:true);

             await datalist0.Reload();
              await InvokeAsync(StateHasChanged);
            ///عاوز اجربها فقط بدون لود;
        }
    }
}
