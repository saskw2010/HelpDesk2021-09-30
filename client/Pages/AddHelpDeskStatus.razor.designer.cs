...
{
    public partial class AddHelpDeskStatusComponent : ComponentBase
    {
        ...
        {
            helpdeskstatus = new Testauth.Models.Authenticationconn.HelpDeskStatus(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Testauth.Models.Authenticationconn.HelpDeskStatus args)
        {
            try
            {
                var authenticationconnCreateHelpDeskStatusResult = await Authenticationconn.CreateHelpDeskStatus(helpDeskStatus:helpdeskstatus);
                UriHelper.NavigateTo("help-desk-statuses");
            }
            catch (System.Exception authenticationconnCreateHelpDeskStatusException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new HelpDeskStatus!" });
            }
        }
