...
{
    public partial class IHelpDeskTicketsComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Splitbutton0Click(RadzenSplitButtonItem args)
        {
            if (args?.Value == "csv")
            {
                await Authenticationconn.ExportHelpDeskTicketsToCSV(new Query() { Filter = $@"{grid0.Query.Filter}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "LocationList,ServiceCatglist,ServicesList", Select = "Id,TicketGUID,TicketStatus,TicketDate,TicketDescription,TicketRequesterUser,TicketRequesterEmail,LocationList.locationDesc,ServiceCatglist.ServiceCatgDesc,ServicesList.ServiceDesc,assignTo,AuditCC" }, $"Help Desk Tickets");

            }

            if (args == null || args.Value == "xlsx")
            {
                await Authenticationconn.ExportHelpDeskTicketsToExcel(new Query() { Filter = $@"{grid0.Query.Filter}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "LocationList,ServiceCatglist,ServicesList", Select = "Id,TicketGUID,TicketStatus,TicketDate,TicketDescription,TicketRequesterUser,TicketRequesterEmail,LocationList.locationDesc,ServiceCatglist.ServiceCatgDesc,ServicesList.ServiceDesc,assignTo,AuditCC" }, $"Help Desk Tickets");
        ...
        protected async System.Threading.Tasks.Task Grid0LoadData(LoadDataArgs args)
        {
            try
            {
                var authenticationconnGetHelpDeskTicketsResult = await Authenticationconn.GetHelpDeskTickets(filter:$@"(contains(TicketGUID,""{search}"") or contains(TicketStatus,""{search}"") or contains(TicketDescription,""{search}"") or contains(TicketRequesterEmail,""{search}"") or contains(assignTo,""{search}"") or contains(AuditCC,""{search}"")) and {(string.IsNullOrEmpty(args.Filter)? "true" : args.Filter)} and TicketRequesterUser eq ""{Security.User.Name}""", orderby:$@"{args.OrderBy}", expand:$"LocationList,ServiceCatglist,ServicesList", top:args.Top, skip:args.Skip, count:args.Top != null && args.Skip != null);
        ...
        {
            UriHelper.NavigateTo($"i-edit-help-desk-ticket/{args.Id}");
        }
